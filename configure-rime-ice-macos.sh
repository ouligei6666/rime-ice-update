...
check_dependencies() {
    ...
    if [ ! -f "$SQUIRREL_APP" ]; then
        log_warning "未检测到 Squirrel 输入法，请确保已正确安装"
        log_info "可以从以下地址下载安装："
...
# 重新加载Squirrel配置
reload_squirrel() {
    log_info "重新加载Squirrel配置..."
    
    if [ -f "$SQUIRREL_APP" ]; then
        # 尝试重新加载配置
        "$SQUIRREL_APP" --reload 2>/dev/null || {
            log_warning "无法自动重新加载Squirrel，请手动重启Squirrel"
            log_info "或者重新登录系统以生效"
        }
        log_success "Squirrel配置重新加载完成"
    else
...
show_config_info() {
    ...
    echo "  输入法: Squirrel (鼠须管)"
    echo ""
    log_info "配置完成后，请重启Squirrel或重新登录以生效"
    log_info "可以通过以下方式重启Squirrel："
